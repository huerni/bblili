...
func NewAddVideoComsumer(c config.Config, ctx context.Context, svcCtx *svc.ServiceContext) {
	client := myrocketmq.RegisterComsumer([]string{c.RocketMQ.NameServer}, constant.BBLILI_ROCKETMQ_GROUP)
	err := client.Subscribe(constant.ROCKETMQ_ADDVIDEO_TOPIC, consumer.MessageSelector{}, func(ctx context.Context, msgs ...*primitive.MessageExt) (consumer.ConsumeResult, error) {
		indexName := "videos"
		...
		var videoinfo search.VideoInfo
		err = json.Unmarshal(msgs[0].Body, &videoinfo)
		if err != nil {
			log.Fatal(err)
		}
